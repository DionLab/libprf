...
PFIMPEXP int ReadProfile(const char * const restrict FileName, struct Profile * const prf,
...
PFIMPEXP int GenerateSequences(const struct Profile * const prf, const float MaxEponent,
                               const char * const FileName, const long int Seed,
                               const enum GeneratedSequenceOptions Options, const unsigned int N);
...
PFIMPEXP int ComplementAlphabet(struct Profile * const prf, const enum ComplementaryMode CplmMode);
PFIMPEXP void PrintJSON(const struct Profile * const prf, FILE* out);

#ifndef __USE_INLINE_FUNCTIONS__
PFIMPEXP void InitializeDefault(union Scores * const matrices, char * const MatchSymbol,
                                char * const InsertionSymbol, const size_t AlphabetMemorySize);
