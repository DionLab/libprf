...
PF_STATIC_INLINE(void) InitializeDefault(union Scores * const matrices,
                                         ...
                                         const size_t AlphabetMemorySize)
{
  /* MATCHES */
  memset(matrices->Match.Alphabet, 0, (AlphabetMemorySize)*sizeof(StoredIntegerFormat));
  /* INSERTIONS */
  memset(matrices->Insertion.Alphabet,    0, (AlphabetMemorySize)*sizeof(StoredIntegerFormat));
...
PF_STATIC_INLINE(int) ComputeHeuristicCutoff(struct Profile * const prf, const int Mode, const int CutOff)
{
