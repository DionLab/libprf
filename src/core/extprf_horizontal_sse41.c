 ...
 */
{
  ...
  const size_t prfLength = prf->Length;
	...
	__m128i* const restrict MatrixPtr = (__m128i*) matrix;
   ...
   */  
  {
    register const StoredIntegerFormat * restrict lMatch      = &Match[0];
		register const StoredIntegerFormat * restrict lInsertion  = &Insertion[0];
    register const ScoreTuple * restrict FirstSequenceProtein = prf->Scores.Insertion.FirstSequenceProtein;
		register const size_t j1 = (size_t) Sequence[0];
    /*
     * PROFILE COLUMN 0 entrance
     */
    int KM = (int) FirstSequenceProtein[0].To[MATCH]     + (int) lMatch[j1];
    int KI = (int) FirstSequenceProtein[0].To[INSERTION] + (int) lInsertion[j1];
    KOPD   = (int) FirstSequenceProtein[0].To[DELETION]  + (int) lMatch[_D];
		
		IOP_W[0].Element.Match     = (KM << SCORE_SHIFT) | PRIORITY_EXTRA;
		IOP_W[0].Element.Insertion = (KI << SCORE_SHIFT) | PRIORITY_EXTRA;
    ...
    for (size_t iprf=0; iprf<prfLength; ++iprf ) {
      register const int KD = KOPD + (int) lMatch[_D];
			
			__m128i __KMI = _mm_setzero_si128();
			_mm_insert_epi32(__KMI, (int) lMatch[j1], MATCH);
			_mm_insert_epi32(__KMI, (int) lInsertion[j1], MATCH);
      ...
      __m128i __FirstSequenceProtein = LoadStoredIntegerVector(&(FirstSequenceProtein[iprf]));
			__FirstSequenceProtein = _mm_add_epi32(__FirstSequenceProtein, __KMI);
      ...
      // Store all scores to matrix
      operation(__max, MatrixPtr, 0, iprf, prfLength);
  ...
  // Swap and assign Read and write pointers
  IOP_R = IOP_W;
  IOP_W = (union sIOP*) (((uintptr_t) &WORK[2*(prf->Length)] + 63) & ~63);

  /*
   * LOOP THROUGH THE SEQUENCE STRING -----------------------------------------------------------------------------
   */
  for ( size_t iseq=1UL; iseq < LSEQ-1UL; ++iseq) {
    register const size_t j1 = (size_t) Sequence[iseq];
    int KOPM = IOP_R[iseq].Element.Match;
    int KOPI = IOP_R[iseq].Element.Insertion;
    register const StoredIntegerFormat * restrict lInsertion = &Insertion[iseq*AlignStep];
		register const StoredIntegerFormat * restrict lMatch     = &Match[iseq*AlignStep];
    /*
     * PROFILE COLUMN iseq entrance
     */
    {
			const int KM = KOPM + (int) lMatch[j1];
			// Transform KM into a vector
      __m128i __KM = _mm_set1_epi32(KM);
      // Load Transitions and Convert signed WORD into signed DWORD
      __m128i __TransitionsM = LoadStoredIntegerVector(&(Transitions[iseq].From[MATCH]));
      // Add KM to Transition
      __TransitionsM = _mm_add_epi32(__TransitionsM, __KM);
      ...
      __m128i __KI = _mm_set1_epi32(KI);
      // Load Transitions and Convert signed WORD into signed DWORD
      __m128i __TransitionsI = LoadStoredIntegerVector(&(Transitions[iseq].From[INSERTION]));
      // Add KI to Transition
      __TransitionsI = _mm_add_epi32(__TransitionsI, __KI);
      ...
      // Get maximum ( this is SSE 4.1 )
      __m128i __max = _mm_max_epi32(__TransitionsM, __TransitionsI);
			
			// Store all scores to matrix
      operation(__max, MatrixPtr, iseq, iseq, prfLength);

      // Clean extra bits
      __max = _mm_srai_epi32(__max, SCORE_SHIFT);

      // Store IOPI and IOPM
      StoreMatchInsertion( &(IOP_W[iseq].mm), (__m128) __max);
     ...
     * LOOP THROUGH THE REST OF THE PROFILE
     */
    for (size_t iprf=(iseq+1UL); iprf<prfLength; ++iprf ) {
      const int KM = IOP_R[iprf].Element.Match + (int) lMatch[j1];
      ...
      // Get maximum ( this is SSE 4.1 )
      __max = _mm_max_epi32(__TransitionsD, __max);

      // Store all scores to matrix
      operation(__max, MatrixPtr, iseq, iprf, prfLength); //_mm_store_si128(&pmatrix[iprf-1], __max1);
   ...
   */
  {
    register const StoredIntegerFormat * restrict lInsertion = &Insertion[LSEQ-1];
		register const StoredIntegerFormat * restrict lMatch = &Match[LSEQ-1];
		register const ScoreTuple * const restrict LastSequenceProtein = prf->Scores.Insertion.LastSequenceProtein;
		const int j1 = (int) Sequence[LSEQ-1];
    
		/* Set NLOW to useless scores before column LSEQ */
		{
			const __m128i __NLOW = _mm_set1_epi32((NLOW << 2) | PRIORITY_EXTRA);
			for (size_t iprf=0UL; iprf<(LSEQ-1UL); ++iprf) operation(__NLOW, MatrixPtr, LSEQ-1, iprf, prfLength);
		}
		
		
		/*
     * PROFILE COLUMN LSEQ entrance
     */
		{
			const int KM = IOP_R[LSEQ-1].Element.Match + lMatch[j1];
			const int KI = IOP_R[LSEQ-1].Element.Insertion + lInsertion[j1];
			
			// Transform KM into a vector
			__m128i __KM = _mm_set1_epi32(KM);
			// Load Transitions and Convert signed WORD into signed DWORD
			__m128i __TransitionsM = LoadStoredIntegerVector(&(Transitions[LSEQ-1].From[MATCH]));
			// Insert LastProteinSequence
			__TransitionsM = _mm_insert_epi32(__TransitionsM, (int) LastSequenceProtein[LSEQ-1].From[MATCH], DUMMY);
			// Add KM to Transition
			__TransitionsM = _mm_add_epi32(__TransitionsM, __KM);
			...
			__m128i __KI = _mm_set1_epi32(KI);
			// Load Transitions and Convert signed WORD into signed DWORD
			__m128i __TransitionsI = LoadStoredIntegerVector(&(Transitions[LSEQ-1].From[INSERTION]));
			// Insert LastProteinSequence
			__TransitionsI = _mm_insert_epi32(__TransitionsI, (int) LastSequenceProtein[LSEQ-1].From[INSERTION], DUMMY);
			// Add KI to Transition
			__TransitionsI = _mm_add_epi32(__TransitionsI, __KI);
			...
			// Store all scores to matrix
			operation(__max, MatrixPtr, LSEQ-1, LSEQ-1, prfLength);
     ...
     * LOOP THROUGH THE REST OF THE PROFILE
     */
    for (size_t iprf=LSEQ; iprf<prfLength; ++iprf) {
      const int KM = IOP_R[iprf].Element.Match + (int) lMatch[j1];
      const int KI = IOP_R[iprf].Element.Insertion + (int) lInsertion[j1];
      const int KD = KOPD + lMatch[_D];
      ...
      // Store all scores to matrix
      operation(__max, MatrixPtr, LSEQ-1, iprf, prfLength);
