...
static void fpga_h_sse41(const struct Profile * const restrict prf,
 ...
 */
{
   ...
   */  
  {
    ...
    IOP_W[0].Element.Insertion = (int) NLOW; //FirstSequenceProtein[0].To[INSERTION];
		const __m128i __NLOW = _mm_set1_epi32((((unsigned int) NLOW) << 2) | PRIORITY_EXTRA);

    {
			__m128i __FirstSequenceProtein = _mm_insert_epi32(__NLOW,
			                                                  ((int) FirstSequenceProtein[0].To[MATCH] << 2) | PRIORITY_EXTRA, MATCH);
    ...
    for (size_t iprf=1; iprf<=prfLength; ++iprf ) {
      // Store all scores to matrix
      operation(__NLOW, MatrixPtr - (1+prfLength), 0, iprf, 1+prfLength);
      
      // Store IOPI and IOPM
			IOP_W[iprf].Element.Match = NLOW;
			IOP_W[iprf].Element.Insertion = NLOW;
    }
  }
  ...
  for ( size_t iseq=BSEQ; iseq < LSEQ-1; ++iseq) {
     ...
     */
    {
      ...
      __m128i __TransitionsX = LoadStoredIntegerVector(&(Transitions[0].From[EXTRA]));
			__TransitionsX = _mm_insert_epi32(__TransitionsX, NLOW, INSERTION);
      ...
      // Store all scores to matrix
      operation(__TransitionsX, MatrixPtr, iseq, 0, 1+prfLength); //_mm_store_si128(&pmatrix[iprf-1], __max1);

      // Clean extra bits
      __TransitionsX = _mm_srai_epi32(__TransitionsX, 2);

			// Store IOPI and IOPM
      StoreMatchInsertion( &(IOP_W[0].mm), (__m128) __TransitionsX);
    ...
    for (size_t iprf=1; iprf<=prfLength; ++iprf ) {
      ...
      // Get maximum ( this is SSE 4.1 )
      __max1 = _mm_max_epi32(__max1, __TransitionsD);
...
//       lScore = _mm_extract_epi32(__max1, DUMMY);
   ...
   */
  {
     ...
     */
    int KOPM     = IOP_R[0].Element.Match;
    KOPD = (int) Transitions[0].Element[_XD];
    ...
    for (size_t iprf=1; iprf<=prfLength; ++iprf) {
      ...
      const int KI = IOP_R[iprf].Element.Insertion + lInsertion[j1];
...
const Compute_t FPGA_sse41 = {
	.BuildMatrix = fpga_h_sse41,
