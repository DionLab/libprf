...
/* Check that position of DELETION is possible */
#if DELETION == 0
#define _mm_insert_epi32(vector, value, pos) _my_insert_epi32_POS0(vector, value)
#define _mm_extract_epi32(vector, pos) _my_extract_epi32_POS0(vector)
#elif DELETION == 1
#define _mm_insert_epi32(vector, value, pos) _my_insert_epi32_POS1(vector, value)
#else
#error "The DELETION position is not possible as is for SSE 2."
#endif
...
extern __inline int __ALWAYS_INLINE
_my_extract_epi32_POS0(const __m128i __A)
{
  return _mm_cvtsi128_si32(_mm_and_si128(__A, _mm_set_epi32(0,0,0,-1)));
}

extern __inline __m128i __ALWAYS_INLINE
_my_max_epi32(__m128i __A, const __m128i __B)
{
	const __m128i __sign = _mm_cmpgt_epi32(__B, __A);
	return _mm_or_si128(_mm_and_si128(__B,__sign), _mm_andnot_si128(__A, __sign));
}
#endif
