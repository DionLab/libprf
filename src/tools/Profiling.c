...
static const char opt_to_test[] = "P:ac:H:I:i:t:hs0:o:Vw!:X:FD:em,Qd:"
...
static void __attribute__((noreturn)) Usage(FILE * stream)
{
  fputs(
	...
	" Profiling [--prf file] [options] [Database, - ]\n"
#else
	" Profiling [--prf file|--regex {pattern}] [options] [Database, - ]\n"
#endif
	" Options:\n"
	"  Sequence\n"
...
int main (int argc, char *argv[])
{
  ...
  char * AffinityMaskFileName;				/* Name of affinity mask file provided by option m */
	...
	const char * Model = NULL;					/* Experiment new models */ 
	const Compute_t * ComputeModel = NULL;
      ...
      case 'i':
...
#ifdef PRF_OUTPUT_PDF
			case 'w':
				OutputType.Specific.PDF.WholeSequence = true;
				break;
#endif
			case 'u':
			...
			case 'r':
				if (OutputType.Type == PDF) {
					const int count = sscanf(optarg, "[%u,%u][%u,%u]", &OutputType.Specific.PDF.y[0], &OutputType.Specific.PDF.x[0],
						                                                 &OutputType.Specific.PDF.y[1], &OutputType.Specific.PDF.x[1]);
  ...
  if (optind >= argc) {
	...
	if (Model) {
		if (OutputVerbose) fprintf(stderr, "Experimenting Model \'%s\'\n", Model);

		if (strcmp(Model, "caller") == 0) {
#ifdef PRF_CORE_EXT_PROFILE
			if (System.Extensions & MM_SSE41) {
				ComputeModel = &ExtendedProfile_sse41;
			}
			else {
				fputs("Caller model requires SSE 4.1...\n", stderr);
				goto bail;
			}
#else
			fprintf(stderr, "Caller model not built...\n", stderr);
			goto bail;
#endif
		}
	}
  ...
  int ProfileCount;
	if (OutputType.SearchWith == PRF) {
	  ...
	  */
	  ...
	  if (prf->Type == PF_MATRIX) {
			/* Default to mode 1, level 0 */
			if (OutputVerbose) fputs("Setting profile to mode 1, level 0\n", stderr);
			if(SetProfileLevelAndMode(prf, 0, 1) != 0) {
				fprintf(stderr, "Unable to set profile to mode 1, level 0, does it exist?\n");
				exit(1);
			}
			/* CutOff */
	    if (Cutoff >= 0) prf->CutOff = Cutoff;
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/* ---------------------------------  FASTA SEQUENCES --------------------------------------- */
		...
		if (ComputeModel == NULL) {
			...
			res = dispatchFASTAFileExt(prf, &FASTA, ComputeModel, 
#ifdef PRF_CORE_PCRE
															   regex,
#endif
															   &OutputType, nCPUs);
