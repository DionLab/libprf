															 ...
															 const _Bool CompleteCycleOnly)
{
  ...
  InitializeDefault(&DefaultScores, &DefaultMatchSymbol, &DefaultInsertionSymbol, ALPHABET_MEMORY_SIZE);
...
void PrintJSON(const struct Profile *const prf, FILE *const out) {
	fputs("{\n", out);
	fprintf(out, "\t\"ac\": \"%s\",\n", prf->AC_Number);
	...
	for (size_t i = 0; i < ALPHABET_SIZE; ++i) {
			if (prf->Alphabet_Mapping[i] != 0) {
					fputc((int) 'A' + i, out);
					AlphabetSize++;
			}
	}
	fputs("\",\n", out);
	//fprintf(out, "\t\"alphabet size\": %i,\n", AlphabetSize); // this can be retrieved from alphabet
	fprintf(out, "\t\"cutoff\": %i,\n", prf->CutOff);
	fprintf(out, "\t\"seek\": \"%s\",\n", prf->Sequence);

	const StoredIntegerFormat *restrict MatchScores = prf->Scores.Match.Alphabet;
	const StoredIntegerFormat *restrict InsertionScores = prf->Scores.Insertion.Alphabet;
	const size_t LeadingDimension = prf->Scores.Insertion.AlignStep;
	static const char *ScoreFormat[] = {"%i", "%i,"};

	const TransitionScores *restrict Transitions = prf->Scores.Insertion.Transitions;


	fputs("\t\"columns\": [\n", out);
	for (size_t iprf = 0UL; iprf <= prfLength; iprf++) {
			fputs("\t\t{\n", out);
			/* ------------------------------- INSERTION ----------------------------- */
			fprintf(out, "\t\t\t\"idx\": %lu,\n", iprf);
			fputs("\t\t\t\"insertion\": [ ", out);
			...
			for (size_t i = 0; i < ALPHABET_SIZE; ++i) {
					...
					if (prf->Alphabet_Mapping[i] != 0) {
							if (++count == AlphabetSize)
									ptrFormat = ScoreFormat[0];
							else
									ptrFormat = ScoreFormat[1];
							fprintf(out, ptrFormat, InsertionScores[prf->Alphabet_Mapping[i]]);
					}
			}
			fputs(" ],\n", out);
			InsertionScores += LeadingDimension;

			/* -------------------------------- DELETION ----------------------------- */
			fprintf(out, "\t\t\t\"deletion\": %i,\n", MatchScores[_D]);
					...
					for (size_t i = 0; i < ALPHABET_SIZE; ++i) {
							...
							if (prf->Alphabet_Mapping[i] != 0) {
									if (++count == AlphabetSize)
											...
											ptrFormat = ScoreFormat[1];
									fprintf(out, ptrFormat, MatchScores[prf->Alphabet_Mapping[i]]);
							}
					}
					fputs(" ],\n", out);
					fprintf(out, "\t\t\t\"unknown\": %i,\n", MatchScores[_UNKNOWN]);
					MatchScores += LeadingDimension;
			...
			fprintf(out, "\t\t\t\t\"MM\": %i, ", (int) Transitions->From[MATCH].To[MATCH]);
			fprintf(out, "\"MI\": %i, ", (int) Transitions->From[MATCH].To[INSERTION]);
			fprintf(out, "\"MD\": %i,\n", (int) Transitions->From[MATCH].To[DELETION]);

			fprintf(out, "\t\t\t\t\"IM\": %i, ", (int) Transitions->From[INSERTION].To[MATCH]);
			fprintf(out, "\"II\": %i, ", (int) Transitions->From[INSERTION].To[INSERTION]);
			fprintf(out, "\"ID\": %i,\n", (int) Transitions->From[INSERTION].To[DELETION]);

			fprintf(out, "\t\t\t\t\"DM\": %i, ", (int) Transitions->From[DELETION].To[MATCH]);
			fprintf(out, "\"DI\": %i, ", (int) Transitions->From[DELETION].To[INSERTION]);
			fprintf(out, "\"DD\": %i\n", (int) Transitions->From[DELETION].To[DELETION]);
