...
{
  size_t localCount = nCycle, localmemorySize;
  char * localMemory;
  char * * localregexString;
  ...
  if (localregexString == NULL) return -4;
	...
	const size_t regexSourceSize = strlen(regexSource);
	if (regexSourceSize == 0) return -1;
	localmemorySize = (1+regexSourceSize) * (((nCycle+1)*nCycle)/2);
  localMemory = (char*) calloc(localmemorySize,sizeof(char));
  if (localMemory == NULL) {
    free(localregexString);
    return -4;
  }
  ...
  /* Compile the regex sources */
  char errorBuffer[1024];
	char * cptr = localMemory;
	for (size_t i=0; i<localCount; ++i) {
		localregexString[i] = cptr;
		for (size_t j=0; j<=i;j++) {
			memcpy(cptr, regexSource, regexSourceSize*sizeof(char));
			cptr += regexSourceSize;
		}
		
// 		fprintf(stderr, "Regex %lu: \'%s\'\n", i, localregexString[i]);
#if defined(PRF_CORE_PCRE)
    const char *error;
    int erroffset;
    localregexCompiled[i] = pcre_compile (localregexString[i],  /* the pattern */
